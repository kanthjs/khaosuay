...
Use this skill when working with R packages to ensure proper development
workflows, testing patterns, and documentation standards.
...
**Preferred expectations:** - `expect_identical()` \> `expect_equal()`
(when exact match expected) - Multiple `expect_true()` calls \> stacking
conditions with `&&` - `expect_s3_class()` \>
`expect_true(inherits(...))` - Use specific `expect_*` functions: -
`expect_lt()`, `expect_gt()`, `expect_lte()`, `expect_gte()` -
`expect_length()` - `expect_named()` - `expect_type()`
...
**Documentation structure:** - One sentence per line in descriptions -
Max 80 characters per line - Use `@family` tags for related functions -
Use `@examples` or `@examplesIf` for examples - UK English spelling
...
Typical `.pre-commit-config.yaml` includes: - `style-files`: Auto-format
R code - `lintr`: Lint R code - `readme-rmd-rendered`: Ensure README.md
is up-to-date - `parsable-R`: Check R syntax - `deps-in-desc`: Check
...
Many R packages use `data.table` for performance: - Functions often
expect/return `data.table` objects - Use
[`data.table::setDT()`](https://rdrr.io/pkg/data.table/man/setDT.html)
or custom `coerce_dt()` to ensure input is data.table - Set keys for
efficient joins: `data.table::setkey(dt, col)` - Use `:=` for in-place
modification

### S3 Classes

- Check class with [`inherits()`](https://rdrr.io/r/base/class.html) or
  `expect_s3_class()`
...
Activate this skill when: - Developing R packages - Writing R tests -
Documenting R functions - Setting up R package infrastructure - Running
R package checks - Working with devtools, testthat, or roxygen2

This skill provides R-specific development patterns. Project-specific
architecture and domain knowledge should remain in project CLAUDE.md
files.
